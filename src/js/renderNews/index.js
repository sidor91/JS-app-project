...
export default class RenderNews {
  ...
  renderNewsCard(
    { imageURL, category, title, desc, date, url },
    additionalClass = '',
  ) {
    ...
    const markup = `<li class="list-news__item popular-news__item">
  <article class="item-news__article">
    <div class="item-news__wrapper-img">
      <img
        class="item-news__img"
  ...
  renderNewsList(newsArr, elemToJoin) {
    ...
    newsMarkup.splice(this.weatherIndex, 0, this.weatherMarkup);
    const newsMarkupWithWeather = newsMarkup
      .map(news => {
        return news;
      })
  ...
  renderNewsbyASearchQX(newsArr, elemToJoin) {
  ...
  renderCategoryNewsQX(newsArr, elemToJoin) {
    console.log(newsArr);
    if (mobileScreenSize === true) {
      this.lastElem = this.currentPage * 4;
      this.firstElem = this.lastElem - 4;
